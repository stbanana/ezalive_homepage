...
function ProductCardItem({
    ...
    return (
        <a
            ...
            className={`animated-border group relative flex h-full flex-col overflow-hidden rounded-sm border bg-fd-card/92 p-6 shadow-md motion-reduce:transition-none ${
                isFeatured
                    ? 'col-span-4 md:col-span-8 lg:col-span-8 lg:p-7'
                    : 'col-span-4 md:col-span-4 lg:col-span-4'
            } ${
                isVisible
                    ? 'translate-y-0 opacity-100 transition-[opacity,transform] duration-500 ease-out'
                    : 'translate-y-8 opacity-0 transition-[opacity,transform] duration-500 ease-out'
            } ${
                isFeatured ? 'border-fd-border/75' : 'border-fd-border/90'
        ...
        >
            ...
            <div className="relative mb-5 space-y-1 text-[11px] text-fd-muted-foreground">
                ...
                {isFeatured && <div>{labels.communicationInterfaces}: {product.communicationInterfaces.join(' / ')}</div>}
                {isFeatured && <div>{labels.communicationProtocols}: {product.communicationProtocols.join(' / ')}</div>}
            </div>
...
export default function HomeProductsSection({
    ...
    const [observerKey] = useState(0);
    const pathname = usePathname();
    const viewAllLabel = viewAll.replace(/\s*[→›»]+$/, '').trim();

    useEffect(() => {
        const onPageShow = (event: PageTransitionEvent) => {
            if (event.persisted) setIsVisible(true);
        };
        const onPopState = () => setIsVisible(true);
    ...
    return (
        <section ref={sectionRef} className="relative md:-mx-4 lg:-mx-8">
            <div className="relative mb-6 px-4 pt-1 lg:px-8">
                <p className="mb-2 text-xs font-semibold uppercase tracking-[0.12em] text-fd-primary/70">
                    {locale === 'zh' ? '核心系列' : 'Product Line'}
                </p>
            ...
            <div className="relative grid grid-cols-4 gap-4 md:grid-cols-8 md:px-4 lg:grid-cols-16 lg:gap-8 lg:px-8">
                ...
                {Array.from({ length: placeholderCount }).map((_, index) => {
                    ...
                    return (
                        <div
                            ...
                            style={{ transitionDelay: delay }}
                            className={`relative flex h-full flex-col items-center justify-center rounded-sm border-2 border-dashed border-fd-border/90 bg-fd-card/75 p-6 text-fd-muted-foreground will-change-transform motion-reduce:transition-none col-span-4 md:col-span-4 lg:col-span-4 ${
                                isVisible
